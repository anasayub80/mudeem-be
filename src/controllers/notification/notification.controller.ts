...
const fetchNotification: RequestHandler = async (req, res) => {
    try {
        ...
        const notification = await Notification.find({ user: new mongoose.Types.ObjectId(userId) });
        if (!notification) {
            return ErrorHandler({
...
const updateSeenNotification: RequestHandler = async (req, res) => {
    try {
        const userId = req.user?._id;
        const notification = await Notification.updateMany({ user: new mongoose.Types.ObjectId(userId) }, { $set: { seen: true } });
        if (!notification) {
            return ErrorHandler({
                message: 'Notification not found',
                statusCode: 400,
